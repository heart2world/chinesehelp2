...
class HomebaseController extends AppframeController {
	...
	protected function check_login(){
		...
		if(empty($session_user['id'])){
			...
			redirect(U('User/Login/index',array('redirect'=>base64_encode($_SERVER['HTTP_REFERER']))));
		}
	...
	protected function  check_user(){
	    $user_status=M('teacher')->where(array("id"=>sp_get_current_userid()))->getField("status");
		// if($user_status==2){
		// 	$this->error('您的申请资料被驳回！',U('User/Center/wsall'));
		// }
		
		// if($user_status==0){
		// 	$this->error('您的申请资料审核中，请联系管理员！',U('User/Center/wsall'));
		// }
	}
