...
class ProblemController extends Controller {
	...
	public function pub()
	{
		...
		// 热门问题
		$qrlist =M('question')->where("parentid=0 and isfree=2")->order('isindex desc')->limit(3)->select();
	...
	public function online_select_teacher()
	{
		...
		$list=M('answer')->field("teacherid,count(*)as number")->where("YEARWEEK(date_format(addtime2,'%Y-%m-%d')) = YEARWEEK(now())".$where)->group('teacherid')->order('number desc')->limit(3)->select();
		$list2=M('answer')->field("teacherid,count(*)as number")->where("YEARWEEK(date_format(addtime2,'%Y-%m-%d')) = YEARWEEK(now())-1".$where)->group('teacherid')->order('number desc')->limit(3)->select();
		...
		if(count($list)<3)
		{
			...
			if(count($list2)<3)
			{
				$tlist =M('teacher')->where("isonline=1".$where)->limit(3)->order("number desc")->select();
				foreach ($tlist as $k => $val) {
			...
			}else
			{
				...
				{
					$info =M('teacher')->field('id,honors,star,nicename,truename,headimg,number')->where("id='".$vl['teacherid']."'".$where)->find();
					$tlist[$kk]['id'] =$info['id'];
					...
					$html.="<li class=\"content-item\" data-type=\"requestion\">
								<a href='".U('Home/Problem/detail',array('id'=>$value['id']))."' class=\"app-flex app-vertical-center\">
									...
									<div class=\"student-info app-basis\">
										<h4 style=\"font-size:0.8rem; color:#333\">".$minfo['nicename']."<span style=\"float:right; font-size:.8rem;color:#999\">".date('Y-m-d',$value['addtime'])."</span></h4>
										<h4 style=\"font-size:.8rem;color:#999; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;\">".$value['title']."</h4>
										<h4 style=\"font-size:.8rem;color:#999\">回答满意度:".$value['star']."</h4>
