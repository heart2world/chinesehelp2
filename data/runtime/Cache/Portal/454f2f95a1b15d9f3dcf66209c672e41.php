...
<meta charset="utf-8">
	...
	<style>
		...
		.toolbar .picker-button{ color:#3cc8fe}
...
<body data-app="ChineseBang">
	...
	<section id="app" style="padding-bottom: 2.65rem;">
		<nav class="top-nav app-flex">
			<div :class="{'cur':cur_index == 1}" class="nav-cell app-basis" @click="change_tab($event,'1');">发表资源</div>
			<div :class="{'cur':cur_index == 2}" class="nav-cell app-basis" @click="change_tab($event,'2');">我的资源</div>
		</nav>
		<!--发表资源-->
		<section v-if="cur_index == 1" class=" form-container" v-cloak style="padding:2.25rem 0 1rem">
		<form id="tagforms" class="weui-cells weui-cells_form" method="post" enctype="multipart/form-data">
			<div class="weui-cell">
				<div class="weui-cell__hd"><label class="weui-label">资源名称</label></div>
				<div class="weui-cell__bd">
					<input class="weui-input" type="text" name="title" placeholder="请输入资源名称" maxlength="20" v-model="resource_name">
				</div>
			</div>
			<div class="weui-cell" style="padding:1px 15px">
				<div class="weui-cell__hd"><label class="weui-label">资源类型</label></div>
				<div class="weui-cell__bd">
					<select name="lesson_type" class="weui-select" v-model="select_resource_type" style="padding-left:0">
						<option value="">请选择资源类型</option>
						<option value="名校真题" <?php if($taskinfo['classtype'] == '名校真题'): ?>selected<?php endif; ?>>名校真题</option>
						<option value="题型专练" <?php if($taskinfo['classtype'] == '题型专练'): ?>selected<?php endif; ?>>题型专练</option>
						<option value="重庆题库" <?php if($taskinfo['classtype'] == '重庆题库'): ?>selected<?php endif; ?>>重庆题库</option>
						<option value="全国题库" <?php if($taskinfo['classtype'] == '全国题库'): ?>selected<?php endif; ?>>全国题库</option>
						<option value="学法指导" <?php if($taskinfo['classtype'] == '学法指导'): ?>selected<?php endif; ?>>学法指导</option>
						<option value="美文分享" <?php if($taskinfo['classtype'] == '美文分享'): ?>selected<?php endif; ?>>美文分享</option>
					</select>
				</div>
			</div>
			<div class="weui-cell">
				...
				<div class="weui-cell__bd">
					<input type="number" name="price" pattern="[0-9]*" placeholder="单位（元）；可为'0'" maxlength="5" v-model="resource_price">
				</div>
			</div>
			<div class="weui-cell">
				<div class="weui-cell__hd" style="align-self: flex-start;"><label class="weui-label">资源简介</label></div>
				<div class="weui-cell__bd">
					<textarea class="weui-textarea" rows="3"  placeholder="请输入资源简介（50字以内）" name="brief" maxlength="50" v-model="resource_desc"></textarea>
				</div>
			</div>
			<div class="weui-cell">
				<div class="weui-cell__hd"><label class="weui-label">资源内容</label></div>
				<div class="weui-cell__bd">
			...
			<div class="input-cell" style="margin-bottom:0">
				<div class="input-control app-basis richtext">
					...
					<div id="form-content-wrap" style="min-height:5rem;padding:0 1rem 1rem">
						<?php if(is_array($imgtaudio)): $i = 0; $__LIST__ = $imgtaudio;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i; if($va['type'] == 'audio'): ?><section class="audio-panel pub-box app-vertical-center">
						...
						<?php if($va['type'] == 'text'): ?><section class="pub-text pub-box app-vertical-center app-content-justify">
							<div class="text app-basis"><?php echo ($va["text"]); ?></div>
					...
					<div class="pub-btns app-flex">
						<div class="app-basis text" type="button" @click="upload_content($event,2);"></div>
						<!--<div class="app-basis voice" type="button" @click="upload_content($event,1);"></div>-->
						<div class="app-basis pic" type="button" @click="upload_content($event,3);"></div>
					</div>
				</div>
			</div>
		...
		<section v-if="cur_index == 2" class="mylesson" v-cloak>
			<ul class="listview">
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><li class="list-item">
					<div class="list-top app-flex app-vertical-center app-content-justify" style="position:relative;padding:.7rem 1rem .4rem">
						<h4 style="font-size:.8rem"><?php echo (date('Y-m-d',$va["addtime"])); ?></h4>
						<!--发布后显示移入专题，未发布显示操作-->
						<?php if($va['status'] == 0): ?><h5 class="action" @click="action($event)">操作<i></i></h5>
						...
						<?php else: ?>
						<h5 class="moveTo" @click="moveFn($event,'<?php echo ($va["id"]); ?>',2)">移入专题</h5>
						<div class="ztList">
							<div class="content">
								<div class="tit app-flex">
									<div class="app-basis" @click="moveChose($event,1)">取消</div>
									<div style="color:#3cc8fe" @click="moveChose($event,2,2)">确定</div>
								</div>
								<div class="weui-cells weui-cells_checkbox">
									<a class="weui-cell weui-check__label" href="<?php echo U('Portal/Zhuantis/addzt',array('istype'=>1,'atype'=>2));?>">
										<div class="weui-cell__bd">
					...
					<a href="<?php echo U('Portal/Resource/detail',array('id'=>$va['id']));?>" style="display:block">
						<div class="list-middle">
							<div class="app-flex">
								<div class="list-title ellipsis app-basis" style="font-size:.8rem"><?php echo ($va["title"]); ?></div>
								<h5><?php if($va['status'] == 0): ?>未发布<?php else: ?>已发布<?php endif; ?></h5>
							</div>
							<div class="list-type app-flex">
								<h4>资源类型 :</h4>
								<h5 class="ellipsis"><?php echo ($va["classtype"]); ?></h5>
							</div>
							<div class="list-type app-flex">
								<h4>资源简介 :</h4>
		...
		<div v-if="cur_index == 1" class="price-desc" style="padding:0 1rem">学生付费后，平台将收取<span><?php echo ($option["zypersent"]); ?>%</span>的资金用于教师奖励基金和系统维持，衷心感谢您的支持！</div>
		...
		<div v-if="cur_index == 1" class="btm-btn pub-btm app-flex app-vertical-center app-hrizontal-center">
			...
			<div class="give-score">
				<a href="javascript:void(0);" @click="pub($event,'2018',false);" style="min-width:6rem">直接发布</a>
			</div>
		</div>
		...
		<div v-show="show_layer3" class="select-panel select-item-panel" v-cloak>
			...
			<p :class="{'cur':item_index == 3}" class="append">在此后插入一条</p>
		</div>
		<div class="meng1"></div>
	</section>
</body>
...
$(function () {
	...
	var app = new Vue({
		el:'#app',
		data:{
			cur_index:sessionStorage.getItem("status2") ? sessionStorage.getItem("status2") : 1,
			type_index:1,
			resource_name:'<?php echo ($taskinfo["title"]); ?>',
			select_resource_type:'<?php echo ($taskinfo["classtype"]); ?>',
			resource_desc:'<?php echo ($taskinfo["brief"]); ?>',
			resource_price:'<?php echo ($taskinfo["price"]); ?>',
			data_text:'没有更多了~',
			show_layer:false,
			show_layer2:false,
			show_layer3:false,
		...
		methods:{
			...
			save_to_draft5:function(evt)
			{
				...
				$.showLoading("自动保存中");
				$.ajax({
						url: "<?php echo U('Portal/Resource/fabu5');?>",
						data: tagvals,
						type:'POST',
			...
			action:function(evt){
				...
				var actionUl=$(evt.currentTarget.nextElementSibling);
			...
			moveFn:function(evt,ids,atype){
				...
				if(self.movedata==''){   //没有专题列表，提示新建
					$.confirm({
						...
						text:'您还没有专题，请先新建专题~',
						onOK:function(){
							window.location.href="<?php echo U('Portal/Zhuantis/addzt',array('istype'=>1,'atype'=>2));?>";
						}
					});
				}
				else{
			...
			pub:function(evt,proid,fal,pub){
				if(pub == '' || pub == undefined){
					...
					if(this.select_resource_type == ''){
						$.toast("请选择资源类型",'text');
						return;
					}
					if(this.resource_price == ''){
						$.toast("请设置价格",'text');
						return;
					}
					if(this.resource_desc == ''){
						$.toast("请填写资源简介",'text');
						return;
					}
				}
				...
				$.confirm({
					...
					onOK:function(){
						$.showLoading("正在提交");						
						$.ajax({
	                        url: "<?php echo U('Portal/Resource/fabu');?>",
	                        data: tagvals,
	                        type:'POST',
	                        success: function (data) {
								$.hideLoading();
			...
			upload_text:function(evt,isedit){
				...
				if(self.pend_type == 2){
					...
					'<div class="text">'+self.text+'</div>'+
					'<div class="app-content-justify" style="text-align:right">'+
						'<button class="alternate" type="button" data-type="text"></button>'+
					'</div>'+
					'<input type="hidden" name="content[]" class="text0999" value="'+self.text+'">'+
					'<input type="hidden" name="atime[]" value="0">'+
			...
			jump_to_edit:function(evt,proid){
				...
				setTimeout(function(){
					...
					try{
						...
						if(sessionStorage.getItem("status2")){
							location.href='./index.php?g=Portal&m=Resource&a=index&id='+proid;
						}
					}catch(e){
						console.warn(e);
					}
			...
			del_resource:function(evt,rid){
				//console.log($(evt.target.parentNode.parentNode.parentNode).index());
				$.confirm({
					title:'',
					text:'删除后将不可见~',
					onOK:function(){
						//点击确定删除
						$.toast("删除成功",'text',function(){
							app.cur_index = 2;
							//location.reload();
