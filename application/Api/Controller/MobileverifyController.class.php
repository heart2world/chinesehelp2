...
class MobileverifyController extends Controller{
    ...
    public function send_msg(){
        ...
        $mobile=M('teacher')->where(['mobile'=>$data['send_address']])->count();
		$member=M('teacher')->where(['mobile'=>$data['send_address']])->getfield('id');
        if($mobile && $data['send_type'] ==0){
            $this->ajaxReturn(['state'=>1,'error'=>'此手机号已注册']);
        }
        if(empty($mobile) && $data['send_type'] ==1){
            $this->ajaxReturn(['state'=>1,'error'=>'此手机号未注册']);
        }
        ...
        $smsConf = array(
            ...
            'tpl_id'    => '79024', //您申请的短信模板ID，根据实际情况修改
