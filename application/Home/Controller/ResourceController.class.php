...
class ResourceController extends Controller {
	...
	public function lesson_info()
	{
		...
		// 查看当前用户是否打过分
		$isscoreed =M('taskscore')->where("userid='".$_SESSION['student']['id']."' and taskid='".$info['id']."' and type='2'")->find();
	...
	function docore()
	{
		if(IS_POST)
		{
			...
			{				
				...
				$count =M('taskscore')->where("taskid='$id' and type=2 and userid='".$_SESSION['student']['id']."'")->count();
				if($count > 0)
				{
		...
		if(IS_POST)
		{
			...
			foreach ($list as $key => $va) {
				if($va['userid']==0)
				{
					...
					$tinfo['truename'] ='问学帮官方';
				}else
				{
					$tinfo =M('teacher')->field('id,headimg,truename,questiontype')->where("id='".$va['userid']."'")->find();
				}
