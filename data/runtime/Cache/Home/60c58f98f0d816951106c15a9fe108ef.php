...
<body data-app="ChineseBang">
	...
	<section id="app">
		<!--教师头部信息-->
		<header class="teacher-hd-info" style="border:none;padding:0" >
			...
			<aside class="teacher-detail-info" style="padding:.5rem 1rem;border-bottom:1px solid #f0eff4">
		...
		<section class="content-module" style="padding-bottom: 0;">
			...
			<article class="content-panel">
					...
					<li class="content-item" data-type="mrolesson">
						<a href="javascript:void(0);" style="display:block">
							<div class="content-top app-flex app-vertical-center">
								<h4 class="app-basis" style="#595959;font-size:.9rem"><?php echo ($info["title"]); ?></h4>
							</div>
							<div class="content-inner-item app-flex">
								<h4>微课简介 :&nbsp;</h4>
								<h5 class="app-basis" style="color:#999"><?php echo ($info["brief"]); ?></h5>
							</div>
							<div class="content-inner-item app-flex">
								<h4>微课评分 :&nbsp;</h4>
								<h5 class="app-basis" style="color:#999"><?php if($info['star'] == '0.0'): ?>5.0<?php else: echo ($info["star"]); endif; ?></h5>
							</div>
				...
				<!--微克具体详情内容 语音、图片、文字-->
				<?php if($ismall == 1 || $info['price'] == '0'): ?><section class="lesson-detail-main">
					<!--语音板块-->
					...
					<?php if($va['type'] == 'text'): ?><section class="reply-content">						
						<p><?php echo ($va["text"]); ?>000</p>
					</section><?php endif; ?>					
					<?php if($va['type'] == 'img'): ?><section class="reply-content resource-content" style="padding-bottom: 0.75rem;">						
						<div class="img-wrap">
		...
		<section class="notice-content" style="padding-bottom:2.5rem">
			<?php if(count($relatelist) != 0): ?><div class="title-panel app-flex app-vertical-center app-content-justify" style="border-bottom:1px solid #f0eff4">
				<h4 style="font-size:1rem;font-weight:normal">平台精选</h4>
			</div>
			<ul class="content-list">
				<?php if(is_array($relatelist)): $i = 0; $__LIST__ = $relatelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><li class="content-item" data-type="mrolesson">
					<a href="<?php echo U('Home/Task/lesson_info',array('id'=>$va['id']));?>" style="display:block">
						<div class="content-top1 app-flex app-vertical-center">
							<h4 class="app-basis ellipsis" style="font-size:.9rem"><?php echo ($va["title"]); ?></h4>
						</div>
						<div class="content-inner-item app-flex">
							<h4><?php echo ($va["type"]); ?>类型 : &nbsp;</h4>
							<h5 class="app-basis" style="font-size:.8rem; color:#999"><?php echo ($va["classtype"]); ?></h5>
						</div>
						<div class="content-inner-item app-flex">
							<h4><?php echo ($va["type"]); ?>简介 : &nbsp;</h4>
							<h5 class="app-basis Line2" style="font-size:.8rem; color:#999"><?php echo ($va["brief"]); ?></h5>
						</div>
						<div class="content-inner-item app-flex">
							<h4>微课评分 : &nbsp;</h4>
							<h5 class="app-basis" style="font-size:.8rem; color:#999"><?php if($va['star'] == '0.0'): ?>5.0<?php else: echo ($va["star"]); endif; ?></h5>
						</div>
						<div class="content-info-num2 app-flex app-vertical-center">
			...
			<div class="more">
				<a href="<?php echo U('Home/Task/index',array('type'=>$studenttype));?>">查看更多</a>
			</div><?php endif; ?>
			<?php if($showpay == 0): ?><div class="tip-me">				
				<a href="<?php echo U('Home/Problem/tipoff',array('id'=>$info['id'],'atype'=>1));?>">举报此内容</a>
			</div><?php endif; ?>
		</section>
		
		<!--价格不为0，且未购买-->
		<div class="weui-btn-area">
			<?php if($showpay == 1): ?><div class="weui-btn weui-btn_primary nopay" @click="pay($event);" <?php if($ismall == 1): ?>style="display:none;"<?php endif; ?>>查看微课需支付<?php echo ($info["price"]); ?>元，立即支付</div>
			<?php else: ?>
				...
				<div class="weui-btn weui-btn_primary" @click="add_favar_or_cancel2($event,'<?php echo ($info["id"]); ?>');"><?php endif; ?>
					<div id="iscollection">
						<?php if($iscollect == 1): ?>取消收藏<?php else: ?>收藏微课<?php endif; ?>
					</div>
				</div>	
				<?php if($isscoreed == 0): ?><div class="weui-btn weui-btn_primary">							
					<a href="<?php echo U('Home/Task/give_score',array('id'=>$info['id']));?>" style="display:block">	
		...
		methods:{
			...
			add_favar_or_cancel2:function(evt,taskid){
				...
				if($("#iscollection").html()=='取消收藏'){
					...
					//数据操作
					$.ajax({
						url:"<?php echo U('Home/Task/collect');?>",
						data:{taskid:taskid,type:2},
						type:'POST',
                        success: function (data) {
                        	if(data.status == 1)
                        	{
                        		$("#iscollection").html('收藏微课');
								$.toast("你已取消对TA的收藏",3000);
                        	}
                        }
					})
