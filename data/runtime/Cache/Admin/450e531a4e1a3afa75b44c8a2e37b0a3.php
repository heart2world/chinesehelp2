			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<select name="xueke">
							<option value="">选择类型</option>
							<?php if(is_array($xuekelist)): $i = 0; $__LIST__ = $xuekelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><option value="<?php echo ($va["title"]); ?>"><?php echo ($va["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
				</div>	
				<div class="control-group">
		...
		var app = new Vue({
			...
			methods:{
				add:function () {	
					...
					$.ajax({
						url:'<?php echo U("Recommended/add_post");?>',
						data:tagvals,
						type:"POST",
						dataType:"json",
